<template>
  <div>
    <buttonGroup :title="`${configKey === 'titleSize' ? '标题' : '描述'}倾斜`" :status="`${status[currentStatus]}px`">
      <el-button-group>
        <el-button :class="{
...
import type { VueComType } from '@/types/common.ts'
...
const changeSize = (num: number) => {
  updateStatus(props.configKey, num)
}
</script>
